...
class MainScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('감정분석', style: TextStyle(color: Color.fromARGB(255, 236, 224, 224),  // 앱바 타이틀 텍스트 색상 변경
        ...
        actions: [
          if (brightness.value != Brightness.light)
            IconButton(
              ...
              icon: const Icon(Icons.light_mode_outlined, color: Color.fromARGB(255, 243, 159, 33)), // 낮모드: 앱바 아이콘 버튼 색상 변경
            ),
          if (brightness.value != Brightness.dark)
            IconButton(
              onPressed: () => brightness.value = Brightness.dark,
              icon: const Icon(Icons.dark_mode_rounded, color: Color.fromRGBO(250, 227, 95, 1)), // 밤모드: 아이콘 버튼 색상 변경
            )
        ],
      ),
      body: ListView(
      children: [
        ListTile(
          contentPadding: EdgeInsets.symmetric(horizontal: 110.0, vertical: 100.0), // 좌우 여백 조절
          title: Row(
            children: [
              Icon(
